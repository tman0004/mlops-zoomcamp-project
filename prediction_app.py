...
# write description for app
st.write("""This app predicts whether a passenger survived the Titanic disaster based on various features.
You can input the passenger details in the form below, and the model will predict the survival status.""")
st.write("""You can also view the model monitoring dashboard from the left sidebar to see how the model is performing over time.""")
with st.form("prediction_form"):
    ...
    with col3:
        ...
        cabin = st.selectbox("Cabin Deck", options=['A', 'B', 'C', 'D', 'E', 'F', 'G', 'T', 'U'], index=8)
        embarked = st.selectbox("Embarked", options=['Cherbourg', 'Queenstown', 'Southampton'], index=0)[0]
